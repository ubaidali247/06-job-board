...
app.get('/api/jobs', (req, res) => {
  ...
  if (shouldBeFlaky(FLAKY_CONFIG.slowProbability)) {
    ...
    console.log(`[FLAKY] Slow GET /api/jobs +${delay}ms`);
...
app.post('/api/jobs', (req, res) => {
  if (shouldBeFlaky(FLAKY_CONFIG.errorProbability)) {
    console.log(`[FLAKY] 500 error on POST /api/jobs`);
...
app.listen(PORT, () => console.log('Job Board running on http://localhost:3006 [FLAKY v2]'));
