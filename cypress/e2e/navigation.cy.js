// Navigation Tests - Job Board
describe('Navigation - Job Board', () => {
  beforeEach(() => {
    cy.visit('/');
  });
  ...
  it('loads the homepage successfully', () => {
    ...
    cy.get('.nav-brand').should('contain', 'Job Board');
  });
  ...
  it('navigates to jobs list page', () => {
  ...
  it('cancel button returns to jobs list', () => {
    cy.get('.nav-link[data-page="add"]').click();
    cy.get('#btn-cancel').click();
    cy.get('#page-jobs').should('not.have.class', 'hidden');
  });

  it('shows add new button on jobs list page', () => {
    cy.get('.nav-link[data-page="jobs"]').click();
  ...
  it('api health endpoint returns correct project name', () => {
    cy.request('/api/health').its('body.project').should('eq', 'Job Board');
  });
});
