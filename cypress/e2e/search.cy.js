// Search and Filter Tests - Job Board
describe('Search and Filter - Job Board', () => {
  beforeEach(() => {
    cy.visit('/');
  ...
  it('searching for existing item title shows result', () => {
    cy.get('#jobs-list .item-card').first().find('.item-title').invoke('text').then(title => {
  ...
  it('searching for non-existent term shows empty state', () => {
    ...
    cy.get('#jobs-list').should('contain', 'No items found');
  });
  ...
  it('clearing search restores full list', () => {
    cy.get('#jobs-list .item-card').then($cards => {
      ...
      cy.get('#jobs-list .item-card').should('have.length', total);
    });
  });
  ...
  it('filter by Engineering category works', () => {
    cy.get('#filter-category').select('Engineering');
  ...
  it('api search endpoint filters results', () => {
    cy.request('/api/jobs?search=test').its('body').should('be.an', 'array');
  });

  it('api category filter endpoint works', () => {
    cy.request('/api/jobs?category=Engineering').its('body').should('be.an', 'array');
  });
  ...
  it('api returns all items when no filter applied', () => {
    cy.request('/api/jobs').its('body').should('have.length.gte', 1);
  });
